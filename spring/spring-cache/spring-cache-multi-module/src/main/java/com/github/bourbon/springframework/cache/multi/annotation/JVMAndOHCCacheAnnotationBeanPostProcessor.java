 ...
 * @date 2022/1/7 23:21
 */
public class JVMAndOHCCacheAnnotationBeanPostProcessor extends AbstractCacheAnnotationBeanPostProcessor {
    ...
    public PropertyValues postProcessProperties(PropertyValues pvs, Object object, String beanName) {
        ...
        do {
            ReflectionUtils.doWithLocalMethods(targetClass, method -> {
                JVMAndOHCCache cache = AnnotationHelperUtils.getAnnotation(method, JVMAndOHCCache.class);
                if (ObjectUtils.nonNull(cache) && method.getParameterCount() == 1) {
                    ReflectionUtils.makeAccessible(method);
                    beanFactory.getBean(JVMAndOHCCacheAspect.class).getCache(method, cache, k -> {
                        try {
