...
public final class JVMUtils {
    ...
    private static String getThreadDumpString(ThreadInfo threadInfo) {
        ...
        sb.append(CharConstants.LF);
        int i = 0;
        StackTraceElement[] stackTrace = threadInfo.getStackTrace();
        MonitorInfo[] lockedMonitors = threadInfo.getLockedMonitors();
        for (; i < stackTrace.length && i < 32; i++) {
            StackTraceElement ste = stackTrace[i];
            sb.append("\tat " + ste.toString() + CharConstants.LF);
            if (i == 0 && threadInfo.getLockInfo() != null) {
                switch (threadInfo.getThreadState()) {
                    case BLOCKED:
                        sb.append("\t-  blocked on " + threadInfo.getLockInfo() + CharConstants.LF);
                        break;
                    case WAITING:
                        sb.append("\t-  waiting on " + threadInfo.getLockInfo() + CharConstants.LF);
                        break;
                    case TIMED_WAITING:
                        sb.append("\t-  waiting on " + threadInfo.getLockInfo() + CharConstants.LF);
                        break;
            ...
            for (MonitorInfo mi : lockedMonitors) {
                if (mi.getLockedStackDepth() == i) {
                    sb.append("\t-  locked " + mi + CharConstants.LF);
                }
            }
        }
        ...
        if (locks.length > 0) {
            sb.append("\n\tNumber of locked synchronizers = " + locks.length + CharConstants.LF);
            for (LockInfo li : locks) {
                sb.append("\t- " + li + CharConstants.LF);
            }
        }
        sb.append(CharConstants.LF);
        return sb.toString();
